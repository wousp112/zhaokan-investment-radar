...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate('''() => {
            ...
            document.querySelector('.content').style.paddingBottom='180px';
        ...
        await chapter('03 / 激活前确认，阈值可修改','核对公司、比较关系与公告类别。将跌幅改为3.5%，保留30分钟冷却。',2)
        ...
        await page.locator('.task-section').scroll_into_view_if_needed()
        ...
        await chapter('05 / 为什么提醒','模拟跌幅达到3.8%，超过设定阈值。提醒已保存，同时进入冷却。',3)
        await page.locator('[data-action="audit"]').first.click()
        await expect(page.locator('#audit-body .evidence-card').first).to_be_visible()
        await page.wait_for_timeout(6000)
        await page.get_by_role('button',name='关闭检查记录',exact=True).click()
        ...
        await expect(page.locator('#stat-alerts')).to_have_text('1')
        await chapter('06 / 冷却与去重','跌幅继续扩大至4.5%，同一交易日的同一价格规则不会重复打扰。',7)
        await page.locator('[data-scenario="announcement"]').click()
        await expect(page.locator('#stat-alerts')).to_have_text('1')
        await chapter('07 / 冷却期间，公告仍被保留','冷却期间发现新业绩预告，先保留。推进演示时钟，冷却结束后再提醒。',5)
