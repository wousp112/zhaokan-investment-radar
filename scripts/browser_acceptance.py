...
async def run(base_url: str):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await expect(page.locator('#welcome-dialog')).to_be_visible()
            await page.get_by_role('button', name='先自己看看', exact=True).click()
            await page.reload(wait_until='networkidle')
            await expect(page.locator('#new-alert')).to_be_enabled()
            await expect(page.locator('#welcome-dialog')).not_to_be_visible()
            passed('新访客显示空状态和明确的新建入口')
            await page.locator('#new-alert').click()
            await page.locator('#prompt').fill(PROMPT)
            await page.locator('#ai-options summary').click()
            await expect(page.locator('#use-ai')).not_to_be_checked()
            await page.locator('#ai-options summary').click()
            await page.locator('#company-options summary').click()
            await page.locator('#target').select_option('300750.SZ')
            await page.locator('#parse-button').click()
            await expect(page.locator('#parse-error')).to_contain_text('但下方选了宁德时代')
